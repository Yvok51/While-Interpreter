...
instance Show a => Show (Interpreter a) where
    show (Inter f) = show (f []) -- TODO: maybe a better option is possible?

instance Functor Interpreter where
    fmap f inter = Inter $ \env ->
        case runInterpreter inter env of 
...
readVariable ident = Inter $ \env -> 
    case findVar env ident of 
        Nothing -> Left ("Variable " ++ ident ++ " is not defined. Currently defined variables: " ++ show env)
        Just val -> Right (val, env)

writeVariable :: Identifier -> Value -> Interpreter ()
writeVariable ident val = Inter $ \env ->
    Right ((), insertVar env ident val)
...
-- Right ((),[("I",NumberVal 7),("Cycles",NumberVal 0)])
--
